...
def get_files(dir):
    ...
    latest_file = max(list_files, key=lambda x: os.path.getctime(os.path.join(dir, x)))
...
def update_intervals(file_data, file_name, success):
    ...
    if file_name not in file_data:
        file_data[file_name] = {'interval': 1, 'last_reviewed': str(datetime.now())}
    else:
...
def select_file_for_review(file_data):
    ...
    now = datetime.now()
    due_files = [file for file, data in file_data.items() if datetime.fromisoformat(data['last_reviewed']) + timedelta(days=data['interval']) <= now]
    if due_files:
...
def main():
    ...
    file_path = os.path.join(current_dir, '../tests/fixtures/random_problem/random_problem')
    ...
    for file in list_files:
        if file not in file_data:
            file_data[file] = {'interval': 1, 'last_reviewed': str(datetime.now())}
    ...
    success = get_enquiry()  # Здесь можно добавить логику для оценки успеха
